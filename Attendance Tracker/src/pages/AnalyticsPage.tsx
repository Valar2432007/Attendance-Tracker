...
import { getSubjects, getRecords, getPercentage, getStatus, getUser, getEnrolledSubjects } from "@/lib/attendance";
...
const AnalyticsPage = () => {
  const user = getUser();
  const subjects = (user?.role === "student" ? getEnrolledSubjects(user) : getSubjects()).filter((s) => s.name.trim());
